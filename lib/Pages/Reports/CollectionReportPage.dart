...
import '../../Service/NewReportsApi/DAILY COLLECTION REPORTApi.dart';
...
class _CollectionReportScreenState extends State<CollectionReportScreen> {
  ...
  List<CollectionReportData> tablerColumn = [];
  ...
  callTestApi() async {
    loadingscrn = true;
    tablerColumn = [];
    await CollectionReportApi.getGlobalData(
      fromDate,
      toDate,
    ).then((value) {
  ...
  List<DataColumn> _generateColumns(List<CollectionReportData> valuesx) {
  ...
  List<DataRow> _generateRows(List<CollectionReportData> data) {
    List<DataRow> rows = [];
    ...
    data.forEach((item) {
      ...
      cells.add(DataCell(Container(
          alignment: Alignment.center, child: Text(item.cardCode.toString()))));
      cells.add(DataCell(Container(
          alignment: Alignment.center, child: Text(item.cardName.toString()))));

      cells.add(DataCell(Container(
          alignment: Alignment.center, child: Text(item.jrnlMemo.toString()))));
      cells.add(DataCell(Container(
          alignment: Alignment.centerRight,
          child: Text(item.cashInHand.toString()))));
      cells.add(DataCell(Container(
          alignment: Alignment.centerRight,
          child: Text(item.chequeDeposits.toString().replaceAll('.0', '')))));
      cells.add(DataCell(Container(
          alignment: Alignment.centerRight,
          child: Text(item.bankName.toString()))));
      cells.add(DataCell(Container(
          alignment: Alignment.center,
          child: Text(item.cashInBankDeposits.toString()))));
      cells.add(DataCell(Container(
          alignment: Alignment.centerRight,
          child: Text(config.alignDateT(item.docDate.toString())))));
      cells.add(DataCell(Container(
          alignment: Alignment.centerRight,
          child: Text(item.acctName.toString()))));
      cells.add(DataCell(Container(
          alignment: Alignment.centerRight,
          child: Text(item.seriesName.toString()))));
      cells.add(DataCell(Container(
          alignment: Alignment.centerRight,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        elevation: 0,
        title: Text('DAILY COLLECTION REPORT_R1'),
      ),
